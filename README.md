...
CIではGitHub Actionsのcacheに保存した固定debug署名キーと、ビルドごとに増える `versionCode` を使います。そのため、同じ署名のAPKを入れた後はアンインストールせず上書きアップデートできます。固定署名にする前のAPKを入れている場合だけ、初回のみアンインストールが必要になる可能性があります。
...
- ホーム: 重要気象情報、現在気温、天気、最高/最低、降水確率、降水量、体感、湿度、風、気圧、UV、日の出/日の入、空気質、雨予測、今後48時間、2週間天気、通知設定を集約
