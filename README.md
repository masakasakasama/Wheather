...
Kotlin / Jetpack Compose / Material 3 / Jetpack Glanceで作る個人用Android天気アプリです。APIキー不要のOpen-Meteoを予報に使い、日本国内の現在気温は気象庁アメダス実況、現在降雨は気象庁雨雲レーダーを優先します。
...
- 検索結果の国コードを地点データへ保存し、気象庁の警報・台風・アメダス・雨雲レーダーは日本の地点を選択している場合だけ取得します。海外地点では現地のOpen-Meteo予報だけを表示し、日本の重要気象情報を混在させません。
...
- レーダーで降雨を観測した場合は、Open-Meteoが晴れでもホーム上部、いまの天気、現在時刻の時間表、ウィジェットを雨表示へ上書きします。レーダー取得失敗・観測期限切れは「降雨なし」と扱わず、予報値表示中であることを明記します。
...
- 雨の開始案内は「その15分/1時間の雨量」と「その日の一日合計」を分けて表示します。今日・明日、2週間、詳細、ウィジェットは現在日以降の同じ日次データを使用し、古いキャッシュの先頭日を「今日」とは表示しません。
- 今日と明日の最高・最低気温には前日比を`[+2]`、`[0]`、`[-1]`形式で表示します。比較用にForecast APIの`past_days=1`を取得します。2週間一覧は日付、天気アイコン、最高、最低、最大降水確率の一行表示です。
- 前日比は各気温の横に直接表示し、重複する説明文は表示しません。時間別のモデル予報幅は専用の固定高行へ揃え、丸め後の下限・上限が同じ場合は`28〜28°`のような重複値を表示しません。

- 気象庁アメダスによる現在気温
  - `latest_time.txt`、観測所一覧、最新時刻の観測JSONを使い、地点から30km以内で品質コードが正常な最寄り観測所を選択
  - 観測から20分以内だけを`実況値`として採用し、観測所名、観測時刻、地点からの距離を表示
  - アメダス対象外の海外地点、30km以内に正常観測所がない地点、取得失敗時はモデル推定値へフォールバック
- 複数モデルによる気温予報
  - Open-Meteoから`JMA Seamless`、`ECMWF IFS 0.25°`、`GFS Seamless`の時間別気温を同時取得
  - 5℃を超える外れ値を除外し、モデル中央値または誤差重み付き統合値を時間別・日別・現在推定へ共通利用
  - アメダス実況との差で直近予報を補正し、補正量は18時間で減衰
  - 各モデルの予報を後日のアメダス実況と端末内で継続比較し、3標本以上から地点・予報時間帯別のMAEとバイアスを統合重みへ反映
  - 最高・最低にはモデル間の予報幅を表示し、日次値は統合済み時間別値から再集計して同じ日の矛盾を防止
...
- Open-Meteo Forecast API
  ...
  - 天気コード、降水、湿度、風などはmodels指定なしのOpen-Meteo Best Matchを優先し、取得失敗時だけ `models=jma_seamless` へフォールバック
  - 気温だけは上記3モデルの統合値へ差し替え、複数モデル取得失敗時はBest Match値を維持
...
- 日本の地点を選択中のみ、気象庁 `targetTimes_N1.json` と雨雲レーダータイルの最新時刻を表示
- 日本の地点を選択中のみ、気象庁の警報・注意報JSONと台風 `targetTc.json` による重要気象情報を表示
  - `typhoonNumber`が数字の場合だけ台風番号として表示。`a`、`b`など未採番の熱帯低気圧識別子は番号にせず、`熱帯低気圧`と表示
  - `TD`、`TS`、`STS`、`TY`などAPI内部コードをそのまま表示せず、画面、通知、Google検索で共通の日本語表記を使用
...
- 雨雲: 日本国内の選択地点周辺に気象庁雨雲タイルを重ねて表示。ズーム、上下左右移動、現在地へ戻る操作に対応。海外地点では日本国内のみ対応であることを表示
...
JMA Seamlessは3日後と5日後の最高気温に約-3.7℃の低温バイアスが見られた。この固定期間の比較だけで1モデルへ決め打ちせず、現在のアプリはJMA・ECMWF・GFSの気温を統合する。端末ごとに選択地点の予報と後日のアメダス実況を比較し、地点・予報時間帯ごとの誤差が3標本以上たまると重みとバイアス補正へ反映する。天気コードと降水はOpen-Meteo Best Matchを維持する。この比較は東京の一期間だけなので、季節や地点が変われば結果も変わり得る。

## 制限事項

- 雨雲レーダーは日本国内限定で、気象庁タイル仕様変更に弱いMVPです。取得失敗時はエラー表示にフォールバックします。
- レーダーAPIの時刻はUTCとして解析し、画面では日本時間へ変換して表示します。
- `リアルタイム`は秒単位ではありません。アメダスは約10分間隔の発表で、アプリ表示中も10分間隔で更新します。20分を超えた観測は実況表示に使いません。WorkManagerはAndroidの省電力制御により30分より遅れる場合があります。
- 10分ごとの再取得で予報値が必ず変わるわけではありません。JMA・ECMWF・GFSの予報は各モデルの配信サイクルで更新されます。
- アメダス実況補正と端末内モデル評価は日本国内向けです。海外地点では複数予報モデルの統合値を`推定`として表示します。
...
- 警報・注意報は日本の地点だけが対象です。現在地点に近い府県予報区をアプリ内の座標リストから推定するため、市区町村単位の完全一致ではありません。
- 台風情報は日本の地点を選択中だけ、気象庁が発表中の台風を表示します。海外のサイクロン情報、進路図、詳細諸元は未実装です。
