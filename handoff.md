...
GitHub Actionsで `assembleDebug` は成功し、スマホへ入れられるdebug APK artifactが生成されている。
...
1. GitHub ActionsのArtifactから `PersonalWeather-debug-apk` をダウンロードして実機にインストールする。
2. 実機で以下を確認する。
   - 初回位置情報許可
   - 許可なし時の東京駅フォールバック
   - Open-Meteo更新
...
まず `handoff.md` を読み、GitHub Actions成功RunのArtifactからAPKを実機インストールして動作確認する。実機で確認できた問題を優先して直す。雨雲レーダー改善より前に、通常予報・キャッシュ・ウィジェットの実機安定性を優先する。
