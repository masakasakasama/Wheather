...
GitHub Actionsでdebug APKをビルドし、`latest-debug` Releaseへ上書きアップロードする構成。固定debug署名キーと増加する `versionCode` をCIで設定しているため、同じ署名のAPK導入後はアンインストールせず上書き更新できる。
